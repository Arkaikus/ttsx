...
def clone(
    text: Annotated[str | None, typer.Argument(help="Text to synthesize (use '-' for stdin)")] = None,
    profile: Annotated[str | None, typer.Option("--profile", "-p", help="Saved voice profile name")] = None,
    audio: Annotated[
        Path | None,
    ...
    try:
        if text_file:
            ...
            console.print("[red]Error:[/red] Provide text via argument, --text-file, or stdin ('-')")
            raise SystemExit(1)
