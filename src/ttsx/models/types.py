...
def get_model_size(model: ModelInfo, fetch_accurate: bool = True) -> int | None:
    ...
    if hasattr(model, "siblings") and model.siblings:
        try:
            sizes_from_siblings = [getattr(sibling, "size", 0) or 0 for sibling in model.siblings]
...
async def get_model_size_async(model: ModelInfo) -> int | None:
